...
def hash(x):
    #encodes the string into SHA-256 before digesting it into hex format
    str = hashlib.sha256(x.rstrip().encode('utf-8'))
...
#runs through the lists and encrypts each value to see if it is the same as the one input by the user
